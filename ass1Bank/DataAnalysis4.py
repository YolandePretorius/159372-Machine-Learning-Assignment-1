...
def readDataFromFile(filename):
    ...
    np_df = np.array(np_df[1:500])
    
    # print("last column", np_df[::,16:17])
    return np_df

def OneNencodeData(Data):
    colnumner = 0
    # numbercolumns = np.shape(Data)[1]
    AgeArray = Data[:,0]
    ...
    JobZero = np.zeros((np.shape(Data)[0],12));
    ...
    durationArray = Data[:,11]
    
    campaignArray = Data[:,12]
    
    pdaysArray = Data[:,13]
    
    previousArray = Data[:,14]
    
    poutcomeArray = Data[:,15]
    ...
    # for col in Data:
    #     if colnumner in listNumericalData:
    #         array = AddtoArrayCol(array,col)
    #     colnumner+=1    
...
def AddtoArrayCol(NewArrayData,col):
    NewArrayData = np.hstack((NewArrayData,col))
    
    # if np.shape(NewArrayData)[0]== 0:
    #     NewArrayData = col
    # else:
    #     NewArrayData = np.append(NewArrayData,col,axis= 0)
    #
    #
    return NewArrayData
...
'''
encode categorical data into  1-of-N encoding
'''
DataAfterEncoding = OneNencodeData(newData)
